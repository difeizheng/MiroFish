...
class ZepEntityReader:
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        # docker patch: GRAPH_LOCAL_ONLY 或 Zep 失败时从本地图谱缓存读取（Zep 额度耗尽离线模式）
        import os
        local_only = os.environ.get('GRAPH_LOCAL_ONLY', '').lower() in ('1', 'true', 'yes')
        if local_only:
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        # docker patch: GRAPH_LOCAL_ONLY 或 Zep 失败时从本地图谱缓存读取
        import os
        local_only = os.environ.get('GRAPH_LOCAL_ONLY', '').lower() in ('1', 'true', 'yes')
        if local_only:
            cached = self._read_graph_cache_data(graph_id)
    ...
    # ===== Agent 实体质量过滤（docker patch）=====
    # 招股书语言产生的高频通用/占位"实体"，作为 Agent 无意义
    ...
    def _read_graph_cache_data(graph_id: str):
        """读取本地图谱缓存（uploads/graphs/<id>.json），返回 data 字段或 None（docker patch）"""
        try:
    ...
    def _get_degree_map(self, graph_id: str, all_edges=None):
        ...
        if edges is None:
            try:
                from .graph_builder import GraphBuilderService
                cached = GraphBuilderService.read_graph_cache(graph_id)
                edges = cached.get('edges') if cached else None
            except Exception:
                edges = None
        if edges is None:
            return None
