...
class ZepToolsService:
    ...
    def search_graph(
        ...
        # docker patch: GRAPH_LOCAL_ONLY 时跳过 Zep 搜索 API，直接本地关键词搜索
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        # docker patch: GRAPH_LOCAL_ONLY 或 Zep 失败时从本地缓存读取
        if _graph_local_only():
            ...
            if cached is not None:
                ...
                return [NodeInfo(
                    uuid=str(n.get('uuid') or ''), name=n.get('name') or '',
                    labels=n.get('labels') or [], summary=n.get('summary') or '',
                    attributes=n.get('attributes') or {}) for n in nodes_c]
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        if _graph_local_only():
            ...
            if cached is not None:
                ...
                for ed in edges_c:
                    ei = EdgeInfo(
                        uuid=str(ed.get('uuid') or ''), name=ed.get('name') or '',
                        fact=ed.get('fact') or '',
                        source_node_uuid=ed.get('source_node_uuid') or '',
                        target_node_uuid=ed.get('target_node_uuid') or '')
                    if include_temporal:
                        ei.created_at = _norm_temporal(ed.get('created_at'))
                        ei.valid_at = _norm_temporal(ed.get('valid_at'))
                        ei.invalid_at = _norm_temporal(ed.get('invalid_at'))
                        ei.expired_at = _norm_temporal(ed.get('expired_at'))
                    result_c.append(ei)
                return result_c
            logger.warning("GRAPH_LOCAL_ONLY 已开启但本地缓存不存在，回退尝试 Zep")
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        # docker patch: GRAPH_LOCAL_ONLY 时扫描本地缓存查节点
        if _graph_local_only():
            try:
                from .graph_builder import GraphBuilderService
                import json as _json
                cache_dir = GraphBuilderService._GRAPH_CACHE_DIR
