...
class ZepGraphMemoryUpdater:
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        # docker patch: GRAPH_LOCAL_ONLY 时跳过 Zep，直接写本地 JSONL（Zep 额度耗尽离线模式）
        import os
        if os.environ.get('GRAPH_LOCAL_ONLY', '').lower() in ('1', 'true', 'yes'):
            self._write_local_memory(combined_text, platform, len(activities))
            return
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    # docker patch: 失败批次落盘本地 JSONL，记忆不丢失
                    self._write_local_memory(combined_text, platform, len(activities))
    
    def _write_local_memory(self, combined_text: str, platform: str, item_count: int):
        """docker patch: 将活动批次写入本地 JSONL（uploads/graphs/<graph_id>.memory.jsonl）"""
        try:
